...
export function Card({ title, children, isOpen }: CardProps) {
    ...
    return (
        <div className={`border rounded-xl px-3 py-2 border-gray-200 shadow-sm`}>
            <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsOpenState(!isOpenState)}>
                <h2 className="font-bold text-gray-700">{title}</h2>
                <div className="flex items-center space-x-2">
                    <button className="text-gray-500 hover:text-gray-700 focus:outline-none cursor-pointer hover:bg-gray-100 rounded-full p-2" >
